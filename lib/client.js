// schedule — browser half (v0.16.7).
// Three floating cards (每日时间安排 / 目标设置 / 日期规划) with toggle,
// drag, dark-theme, localStorage persistence.
// v0.16.7: FLOW DOCK SELF-HEAL. Shrinking the flowchart (⇥/←/Esc) docks its
//   project-tree outline to the RIGHT of the 目标设置 card (body-level panel,
//   restart-persistent via schedule.flowDock). CRITICAL FIX: goals created by
//   old versions have no `id`, so setItem(FLOW_*_KEY, g.id) stored the literal
//   string "undefined" and items.find(i => i.id === ...) silently missed — the
//   dock never appeared. Boot now assigns missing ids and heals corrupted keys
//   ("undefined" dock key -> first goal with a tree; invalid open key -> drop).
...
// background is a plain dark gradient (v0.16.6 removed the embedded wallpaper).
  ...
  factory: (require) => {
    ...
    function injectStyles() {
      ...
      s.textContent = [
        ...
        // ---------- docked flowchart project tree (floating beside the 目标设置 card) ----------
        ".dww-flowdock{position:fixed;z-index:2147482000;display:flex;flex-direction:column;border:1px solid rgba(30,40,60,.16);border-radius:14px;overflow:hidden;background:rgba(250,252,255,.97);box-shadow:0 18px 48px rgba(8,16,34,.30);font-family:'Microsoft YaHei','PingFang SC','Segoe UI',system-ui,sans-serif;}",
        ".dww-flowdock-head{display:flex;align-items:center;gap:8px;padding:8px 10px;background:#f4f6fa;border-bottom:1px solid rgba(30,40,60,.1);}",
        ".dww-flowdock-title{flex:1;min-width:0;font-size:12px;font-weight:800;color:#3a4a66;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
        ".dww-flowdock-x{flex:0 0 auto;border:none;background:transparent;color:rgba(60,70,90,.75);font-size:15px;line-height:1;cursor:pointer;padding:0 2px;}",
        ".dww-flowdock-x:hover{color:#d9534f;}",
        ".dww-flowdock-body{flex:1;min-height:0;overflow:auto;padding:6px;background:#f7f9fc;}",
        ".dww-flowtree{width:100%;}",
        ".dww-flowtree-row{display:flex;align-items:center;gap:6px;padding:4px 6px;border-radius:7px;cursor:pointer;font-size:12px;color:#2b3550;transition:background .12s;}",
        ".dww-flowtree-row:hover{background:rgba(59,123,255,.14);}",
        ".dww-flowtree-mark{flex:0 0 auto;font-size:11px;color:rgba(60,80,120,.7);}",
        ".dww-flowtree-done .dww-flowtree-mark{color:#2fa96b;}",
        ".dww-flowtree-done .dww-flowtree-text{color:rgba(43,53,80,.55);text-decoration:line-through;}",
        ".dww-flowtree-text{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
        ".dww-flowtree-root .dww-flowtree-text{font-weight:800;}",
    ...
    function goalsBody() {
      ...
      const dockEl = el("div", { class: "dww-flowdock" });
...
// level, regardless of connection depth. Edges carry the hierarchy.
      ...
      // v0.16.7 FIX: goals created by older versions have NO `id`. The flow-state
      // persistence (schedule.flowOpen / schedule.flowDock) locates the goal via
      // items.find(i => i.id === ...) — with idless goals the lookups silently
      // failed and the literal string "undefined" got written into localStorage,
      // so the docked tree sidebar never appeared. Assign missing ids once, then
      // sanitize the two flow-state keys against real ids:
      // - corrupted "undefined" dock key -> HEAL to the first goal with a tree
      //   (the user shrunk the flowchart and expects its tree to hang on the right);
      // - any other unmatched id -> drop the key.
      let idsFixed = false;
      items.forEach((g) => { if (g && !g.id) { g.id = uid(); idsFixed = true; } });
      try {
        const idOk = (id) => id && items.some((i) => i.id === id);
        const dk = localStorage.getItem(FLOW_DOCK_KEY);
        if (dk && !idOk(dk)) {
          const t0 = items.find((i) => i.tree);
          if (dk === "undefined" && t0) localStorage.setItem(FLOW_DOCK_KEY, t0.id);
          else localStorage.removeItem(FLOW_DOCK_KEY);
        }
        const ok = localStorage.getItem(FLOW_OPEN_KEY);
        if (ok && !idOk(ok)) localStorage.removeItem(FLOW_OPEN_KEY);
      } catch (_) {}
      if (idsFixed) persist();
      ...
      // ---------- docked flowchart PROJECT TREE sidebar ----------
      // Any way of shrinking the workspace parks the goal's node tree here: a
      // compact outline floating on the desktop RIGHT NEXT to the 目标设置 card.
      // Click a row to reopen the full workspace; × dismisses the sidebar.
      // The choice survives restarts.
      function buildTreeRows(tree) {
        const rows = [];
        const nodes = {};
        [tree].concat(tree.children || []).forEach((n) => { nodes[n.id] = n; });
        const byParent = {};
        (tree.edges || []).forEach((e) => { (byParent[e.from] = byParent[e.from] || []).push(e.to); });
        const seen = {};
        (function walk(id, depth) {
          if (seen[id]) return;
          seen[id] = true;
          if (nodes[id]) rows.push({ n: nodes[id], depth: depth });
          (byParent[id] || []).forEach((cid) => walk(cid, depth + 1));
        })(tree.id, 0);
        // Orphans never reached via edges go last, one level under the root.
        (tree.children || []).forEach((c) => {
          if (!seen[c.id]) { seen[c.id] = true; rows.push({ n: c, depth: 1 }); }
        });
        return rows;
      }
      // The dock is a body-level panel pinned to the RIGHT EDGE of the 目标设置
      // card — it floats on the desktop NEXT TO the card, not inside it (the card
      // shell has overflow:hidden, so an in-card sidebar would be clipped). A
      // style/class observer keeps it glued to the card while it is dragged or
      // resized, and hides it whenever the card itself is hidden.
      let dockObserver = null, dockRetry = 0;
      function positionDock() {
        const card = document.getElementById("dww-goals");
        if (!card) return false;
        const r = card.getBoundingClientRect();
        dockEl.style.left = Math.min(Math.round(r.right + 10), Math.max(0, innerWidth - 310)) + "px";
        dockEl.style.top = Math.round(r.top) + "px";
        dockEl.style.width = "300px";
        dockEl.style.height = Math.max(160, Math.round(r.height)) + "px";
        dockEl.style.display = card.classList.contains("dww-hidden") ? "none" : "";
        return true;
      }
      function watchDockCard() {
        const card = document.getElementById("dww-goals");
        if (!card || dockObserver) return;
        dockObserver = new MutationObserver(() => positionDock());
        dockObserver.observe(card, { attributes: true, attributeFilter: ["style", "class"] });
      }
      function retryDockPosition() {
        if (dockRetry) return;
        let tries = 0;
        const tick = () => {
          tries++;
          if (positionDock() || tries > 120) { dockRetry = 0; return; }
      ...
      function renderDock() {
        let dockId = null;
        try { dockId = localStorage.getItem(FLOW_DOCK_KEY); } catch (_) {}
        const g = dockId ? items.find((i) => i.id === dockId) : null;
        if (!g || !g.tree) { if (dockEl.parentNode) dockEl.remove(); return; }
        const rows = buildTreeRows(g.tree);
        dockEl.innerHTML = "";
        dockEl.append(
          el("div", { class: "dww-flowdock-head" }, [
            el("span", { class: "dww-flowdock-title", text: "🗺 " + (g.tree.text || g.title || "主目标") + " · " + (g.tree.children || []).length + " 项" }),
            el("button", {
              class: "dww-flowdock-x", text: "×", title: "移除项目树",
              onclick: (e) => { e.stopPropagation(); try { localStorage.removeItem(FLOW_DOCK_KEY); } catch (_) {} renderDock(); }
            })
          ]),
          el("div", {
            class: "dww-flowdock-body", title: "点击任意一项展开完整流程图"
          }, [
            el("div", { class: "dww-flowtree" },
              rows.map(function (r) {
                return el("div", {
                  class: "dww-flowtree-row" + (r.n.done ? " dww-flowtree-done" : "") + (r.depth === 0 ? " dww-flowtree-root" : ""),
                  style: { paddingLeft: (6 + r.depth * 14) + "px" },
                  title: "点击展开完整流程图",
                  onclick: () => openFlow(g, render)
                }, [
                  el("span", { class: "dww-flowtree-mark", text: r.n.done ? "✔" : "○" }),
                  el("span", { class: "dww-flowtree-text", text: r.n.text || "（未命名）" })
                ]);
              })
            )
          ])
        );
        if (dockEl.parentNode !== document.body) document.body.appendChild(dockEl);
        if (!positionDock()) retryDockPosition(); // card may not be mounted yet at boot
        watchDockCard();
      }
      function getDefaultGoal() {
        if (items.length === 0) {
          const g0 = { id: uid(), title: "主目标", pct: 0, tree: { id: uid(), text: "主目标", done: false, shape: "rect", children: [], edges: [] } };
          items.push(g0); persist(); render();
          return g0;
        }
        return items[0];
      }
      flowLauncherOpen = (goalId) => {
        const g = (goalId && items.find((i) => i.id === goalId)) || getDefaultGoal();
        openFlow(g, render);
      };
      function openFlow(g, rerender) {
        ...
        // Remember that a flowchart is open (and for which goal) so it can be
        // restored automatically the next time the widgets boot.
        flowIsOpen = true;
        try { localStorage.setItem(FLOW_OPEN_KEY, g.id); } catch (_) {}
        ...
        // ⇥ minimizes the workspace: the goal's PROJECT TREE docks to the right
        // of the 目标设置 card (closeFlow() always parks it there now).
        const dockBtn = el("button", { class: "dww-flow-maxbtn", text: "⇥", title: "最小化：项目树挂到「目标设置」右侧", onclick: () => closeFlow() });
...
// distance from point (px,py) to an orthogonal polyline made of the given points
        ...
        function closeFlow() {
          ...
          overlay.remove();
          flowIsOpen = false;
          try { localStorage.removeItem(FLOW_OPEN_KEY); } catch (_) {}
          // ANY way of shrinking the workspace (⇥ / ← 返回 / Esc) parks the goal's
          // PROJECT TREE on the right of the 目标设置 card. The × on that sidebar
          // is the only way to dismiss it.
          try { localStorage.setItem(FLOW_DOCK_KEY, g.id); } catch (_) {}
    ...
    // Hook set by goalsBody(): opens the flowchart workspace for a given goal
    // (or the default one when no id is passed).
    let flowLauncherOpen = null;
    // Persists WHICH goal's flowchart is open, so a flowchart left open when the
    // app closes is restored automatically on the next boot. Cleared on close.
    const FLOW_OPEN_KEY = "schedule.flowOpen";
    // Remembers which goal's flowchart is DOCKED into the 目标设置 card as a
    // read-only minimap (the ⇥ button in the flowchart title bar sets it).
    const FLOW_DOCK_KEY = "schedule.flowDock";
    let flowIsOpen = false;
    ...
    function mountFloatingLauncher(cards) {
      ...
      const btn = el("button", {
        class: "dww-launcher",
        id: "dww-launcher",
        title: "🧩 点击：显示/隐藏小组件 · 拖动：移动按钮\n📊 点击：直接打开流程图工作台",
        type: "button"
      }, [
        el("span", { class: "dww-launcher-icon", text: "🧩" }),
        el("span", { class: "dww-launcher-text", text: "小组件" }),
        el("span", { class: "dww-launcher-icon dww-launcher-flow", text: "📊", title: "打开流程图工作台（独立桌面窗口）" })
      ]);
      ...
      // 📊 flowchart launcher: clicking it opens the flow workspace directly,
      // without first opening the "目标设置" card. stopPropagation so the pill
      // doesn't treat it as a toggle/drag gesture.
      const flowIcon = btn.querySelector(".dww-launcher-flow");
      if (flowIcon) {
        flowIcon.style.marginLeft = "7px";
        flowIcon.style.paddingLeft = "9px";
        flowIcon.style.borderLeft = "1px solid rgba(255,255,255,.4)";
        flowIcon.style.cursor = "pointer";
        flowIcon.addEventListener("pointerdown", (e) => e.stopPropagation());
        flowIcon.addEventListener("click", (e) => {
          e.stopPropagation();
          if (typeof flowLauncherOpen === "function" && !flowIsOpen) flowLauncherOpen();
        });
      }
    ...
    function boot() {
      ...
      applyTheme();
      observeTheme();
      // Restore the flowchart workspace if it was left open last session.
      try {
        const openId = localStorage.getItem(FLOW_OPEN_KEY);
        if (openId && typeof flowLauncherOpen === "function" && !flowIsOpen) flowLauncherOpen(openId);
      } catch (_) {}
